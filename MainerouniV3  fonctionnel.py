...
def jet_attaque(bonus_atk):
    assert isinstance(bonus_atk, (int,float)), "Type bonus d'attaque incorrect"
    #begin
    résultat_jet_atk=dé(20)+bonus_atk
    return résultat_jet_atk
    #end

def jet_dégâts(nombre_dés_dégâts,type_dés_dégâts):
    assert isinstance(nombre_dés_dégâts, int), "Type nombre de dés incorrect"
    assert isinstance(type_dés_dégâts, int), "Type 'type' de dés incorrect"
    #déclaration de la variable
    résultat_jet_dégâts=0
    i=nombre_dés_dégâts
    #begin
    while i>0 :
        résultat_jet_dégâts=résultat_jet_dégâts+dé(type_dés_dégâts)
        i=i-1
    return résultat_jet_dégâts
    #end

def donne_classe_de_réussite(résultat_jet_atk,en_dessous_échec,au_dessus_crit):
    #pas besoin de assert puisque résultat_jet_atk forcément int puisque défini par notre fonction
    assert isinstance(en_dessous_échec, int), "Type en dessous échec incorrect"
    assert isinstance(au_dessus_crit, int), "Type au dessus crit incorrect"
    #begin
    if résultat_jet_atk >= au_dessus_crit :
        return 2
    else :
        if résultat_jet_atk > en_dessous_échec :
            return 1
        else :
            return 0
    #end
        
def simple_calcul_dégâts(résultat_jet_dégâts,magic_bonus):
    #pas besoin de assert puisque résultat_jet_dégâts forcément int puisque défini par notre fonction
    assert isinstance(magic_bonus,(int, float)), "Type bonus magique incorrect"
    #begin
    résultat_simple_calcul_dégâts = résultat_jet_dégâts + magic_bonus
    return résultat_simple_calcul_dégâts
    #end

def crit_calcul_dégâts(nombre_dés_dégâts,type_dés_dégâts,magic_bonus,facteur_crit):
    assert isinstance(magic_bonus,(int, float)), "Type bonus magique incorrect"
    assert isinstance(nombre_dés_dégâts, int), "Type nombre de dés incorrect"
    assert isinstance(type_dés_dégâts, int), "Type 'type' de dés incorrect"
    assert isinstance(facteur_crit, int), "Type facteur de crit incorrect"
    #déclaration de la variable
    résultat_crit_calcul_dégâts = 0
    i=facteur_crit
    #begin
    while i>0:
        résultat_crit_calcul_dégâts = résultat_crit_calcul_dégâts+jet_dégâts(nombre_dés_dégâts,type_dés_dégâts)+magic_bonus
        i=i-1
    return résultat_crit_calcul_dégâts
    #end

def attaque(bonus_atk,nombre_dés_dégâts,type_dés_dégâts,magic_bonus,classe_armure_adversaire,facteur_crit):
    assert isinstance(classe_armure_adversaire,(int, float)), "Type classe d'armure incorrect"
    #toutes les autres variables sont testées dans les sous-fonctions
    en_dessous_échec = 2
    au_dessus_crit = 1
    facteur_crit=2
    #begin
    if donne_classe_de_réussite(jet_attaque(bonus_atk),en_dessous_échec,au_dessus_crit)==0:                   #bloc de calcul et retour des attaques non réussies
        print("Votre attaque a échoué")
        return 0
    elif donne_classe_de_réussite(jet_attaque(bonus_atk),en_dessous_échec,au_dessus_crit)==1:                 #bloc de calcul et retour des attaques réussies
        if simple_calcul_dégâts(jet_dégâts(nombre_dés_dégâts,type_dés_dégâts),magic_bonus)-classe_armure_adversaire>1:
            return (simple_calcul_dégâts(jet_dégâts(nombre_dés_dégâts,type_dés_dégâts),magic_bonus)-classe_armure_adversaire)
        else :
            return 1    #car une attaque réussie vaut toujours au moins 1
    elif donne_classe_de_réussite(jet_attaque(bonus_atk),en_dessous_échec,au_dessus_crit)==2:                 #bloc de calcul et retour des attaques critiques
        if crit_calcul_dégâts(nombre_dés_dégâts,type_dés_dégâts,magic_bonus,facteur_crit)-classe_armure_adversaire> 1:
            return (crit_calcul_dégâts(nombre_dés_dégâts,type_dés_dégâts,magic_bonus,facteur_crit)-classe_armure_adversaire)
        else :
            return 1    #car une attaque réussie vaut toujours au moins 1
